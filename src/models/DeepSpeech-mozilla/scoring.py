...
def main():
    ...
    for filename in os.listdir(folder):
        fin = wave.open("/".join([folder, filename]), 'rb')
#        fin = wave.open(args.audio, 'rb')
        ...
        with open("wer_test.txt", "a") as myfile:
            myfile.write("%s," %k)
            if args.extended:
                # print(metadata_to_string(ds.sttWithMetadata(audio, fs)))
                myfile.write(metadata_to_string(ds.sttWithMetadata(audio, fs)))
            else:
                # print(ds.stt(audio, fs))
                myfile.write(ds.stt(audio, fs))
            myfile.write(",%s\n" %audio_length)
            myfile.close()
